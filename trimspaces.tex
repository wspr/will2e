...
\ProvidesPackage{trimspaces}[2009/09/16 v1.0
...
\newcommand\trim@spaces@in[1]{%
  ...
  \expandafter\expandafter\expandafter\expandafter
  \expandafter\expandafter\expandafter
  {\expandafter\trim@spaces\expandafter{#1}}%
}
...
This package provides two macros for the \LaTeX\ programmer:
\begin{itemize}
\item \cs{trim@spaces}\marg{text}
\item \cs{trim@spaces@in}\meta{macro}
\end{itemize}
The former is fully expandable and removes any surrounding spaces from \meta{text}.
The latter re-defines \meta{macro} as itself stripped of any surrounding spaces
(it is obviously not expandable).

For example:
\begin{verbatim}
\edef\a{\trim@spaces{ a a a }}
\def\b{ b \foo b }
\trim@spaces@in\b
This is a: [\meaning\a] and this is b: [\meaning\b]
\end{verbatim}
which produces:
\begin{quote}
\makeatletter
